...
combo <- read_csv("F:/covid19-redeploy/merge-others-output/combined-metrics.csv",
                  col_types = cols(
                    .default = col_double(),
...
expect_true(all(table(combo$Data_Period) == 1200),
            info = "All Periods have 1200 Obs.")

###all periods have 80 summary rows and 1120 daily rows
expect_true(all(table(combo$Data_Period[combo$RowType == "Summary"]) == 80),
            info = "All periods have 80 summary rows.")
expect_true(all(table(combo$Data_Period[combo$RowType == "Daily"]) == 1120),
            info = "All periods have 1120 daily rows.")

###all periods have 14 consecutive days
expect_true(all(combo %>%
...
##. . Common Variables ----
###Date
expect_true(inherits(combo$Date, 'Date'),
            info = "Date column is 'Date' class")
expect_equal(sum(is.na(combo$Date)), 0,
             info = "Date column has no NA/missings")

###Region_ID
expect_true(inherits(combo$Region_ID, 'character'),
            info = "Region_ID column is 'character' class")
expect_equal(sum(is.na(combo$Region_ID)), 0,
             info = "Region_ID column has no NA/missings")
expect_equal(sort(unique(combo$Region_ID)),
                 c("55",
...
###Region
expect_true(inherits(combo$Region, 'character'),
            info = "Region column is 'character' class")
expect_equal(sum(is.na(combo$Region)), 0,
             info = "Region column has no NA/missings")
expect_equal(sort(unique(combo$Region)),
...
###RowType
expect_true(inherits(combo$RowType, 'character'),
            info = "RowType column is 'character' class")
expect_equal(sum(is.na(combo$RowType)), 0,
             info = "RowType column has no NA/missings")
expect_equal(sort(unique(combo$RowType)),
             c("Daily", "Summary"),
             info = "RowType column has only correct unique values")

###Data_Period
expect_true(inherits(combo$Data_Period, 'character'),
            info = "Data_Period column is 'character' class")
expect_equal(sum(is.na(combo$Data_Period)), 0,
             info = "Data_Period column has no NA/missings")
expect_equal(length(unique(combo$Data_Period)), nrow(combo)/1200,
...
expect_true(all(grepl("^([1][0-2]|[1-9])/([1-9]|[1-2][0-9]|3[0-1])/202[0-9] - ([1][0-2]|[1-9])/([1-9]|[1-2][0-9]|3[0-1])/202[0-9]$",
                      unique(combo$Data_Period))),
            info = "Data_Period values all conform to the correct pattern (m/d/202y - m/d/202y)")

##. . Confirmed Cases ----
###Conf_Case_Count
expect_true(inherits(combo$Conf_Case_Count, 'numeric'),
            info = "Conf_Case_Count column is 'numeric' class")
expect_equal(sum(is.na(combo$Conf_Case_Count)), 0,
             info = "Conf_Case_Count column has no NA/missings")

###Conf_Case_Burden
expect_true(inherits(combo$Conf_Case_Burden, 'numeric'),
            info = "Conf_Case_Burden column is 'numeric' class")
expect_equal(sum(is.na(combo$Conf_Case_Burden[combo$RowType == "Summary"])), 0,
             info = "Conf_Case_Burden column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Conf_Case_Burden[combo$RowType == "Daily"])), 0,
             info = "Conf_Case_Burden column has *all* NA/missings for Daily Rows")

###Conf_Case_Trajectory
expect_true(inherits(combo$Conf_Case_Trajectory, 'character'),
            info = "Conf_Case_Trajectory column is 'character' class")
expect_equal(sum(is.na(combo$Conf_Case_Trajectory[combo$RowType == "Summary"])), 0,
             info = "Conf_Case_Trajectory column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Conf_Case_Trajectory[combo$RowType == "Daily"])), 0,
             info = "Conf_Case_Trajectory column has *all* NA/missings for Daily Rows")

###Conf_Case_Burden_Class
expect_true(inherits(combo$Conf_Case_Burden_Class, 'character'),
            info = "Conf_Case_Burden_Class column is 'character' class")
expect_equal(sum(is.na(combo$Conf_Case_Burden_Class[combo$RowType == "Summary"])), 0,
             info = "Conf_Case_Burden_Class column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Conf_Case_Burden_Class[combo$RowType == "Daily"])), 0,
             info = "Conf_Case_Burden_Class column has *all* NA/missings for Daily Rows")
expect_equal(sort(unique(combo$Conf_Case_Burden_Class)),
...
expect_true(inherits(combo$Conf_Case_Trajectory_Class, 'character'),
            info = "Conf_Case_Trajectory_Class column is 'character' class")
expect_equal(sum(is.na(combo$Conf_Case_Trajectory_Class[combo$RowType == "Summary"])), 0,
             info = "Conf_Case_Trajectory_Class column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Conf_Case_Trajectory_Class[combo$RowType == "Daily"])), 0,
             info = "Conf_Case_Trajectory_Class column has *all* NA/missings for Daily Rows")
expect_equal(sort(unique(combo$Conf_Case_Trajectory_Class)),
...
expect_true(inherits(combo$Conf_Case_Composite_Class, 'character'),
            info = "Conf_Case_Composite_Class column is 'character' class")
expect_equal(sum(is.na(combo$Conf_Case_Composite_Class[combo$RowType == "Summary"])), 0,
             info = "Conf_Case_Composite_Class column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Conf_Case_Composite_Class[combo$RowType == "Daily"])), 0,
             info = "Conf_Case_Composite_Class column has *all* NA/missings for Daily Rows")
expect_equal(sort(unique(combo$Conf_Case_Composite_Class)),
             c("High", "Low", "Medium"),
             info = "Conf_Case_Composite_Class column has only correct unique values")

###Conf_Case_Trajectory_P
expect_true(inherits(combo$Conf_Case_Trajectory_P, 'numeric'),
            info = "Conf_Case_Trajectory_P column is 'numeric' class")
expect_equal(sum(is.na(combo$Conf_Case_Trajectory_P[combo$RowType == "Summary"])), 0,
             info = "Conf_Case_Trajectory_P column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Conf_Case_Trajectory_P[combo$RowType == "Daily"])), 0,
             info = "Conf_Case_Trajectory_P column has *all* NA/missings for Daily Rows")
expect_true(all(dplyr::between(combo$Conf_Case_Trajectory_P[combo$RowType == "Summary"], left = 0.0, right = 1.0)),
            info = "Conf_Case_Trajectory_P columns values are all between 0 and 1 inclusive.")

###Conf_Case_Trajectory_FDR
expect_true(inherits(combo$Conf_Case_Trajectory_FDR, 'numeric'),
            info = "Conf_Case_Trajectory_FDR column is 'numeric' class")
expect_equal(sum(is.na(combo$Conf_Case_Trajectory_FDR[combo$RowType == "Summary"])), 0,
             info = "Conf_Case_Trajectory_FDR column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Conf_Case_Trajectory_FDR[combo$RowType == "Daily"])), 0,
             info = "Conf_Case_Trajectory_FDR column has *all* NA/missings for Daily Rows")
expect_true(all(dplyr::between(combo$Conf_Case_Trajectory_FDR[combo$RowType == "Summary"], left = 0.0, right = 1.0)),
...
expect_true(inherits(combo$Testing_Positive_Specimens, 'numeric'),
            info = "Testing_Positive_Specimens column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Positive_Specimens)), 0,
             info = "Testing_Positive_Specimens column has no NA/missings")

###Testing_Nonpositive_Specimens
expect_true(inherits(combo$Testing_Nonpositive_Specimens, 'numeric'),
...
expect_true(inherits(combo$Testing_Total_Specimens, 'numeric'),
            info = "Testing_Total_Specimens column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Total_Specimens)), 0,
             info = "Testing_Total_Specimens column has no NA/missings")

###Testing_Percent_Positive
expect_true(inherits(combo$Testing_Percent_Positive, 'numeric'),
            info = "Testing_Percent_Positive column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Percent_Positive[combo$Testing_Total_Specimens > 0])), 0,
...
expect_true(all(dplyr::between(combo$Testing_Percent_Positive[combo$Testing_Total_Specimens > 0], left = 0.0, right = 100.0)),
            info = "Testing_Percent_Positive columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$Testing_Percent_Positive[combo$Testing_Total_Specimens > 0] > 1.0),
            info = "Testing_Percent_Positive columns values are scales between 0 and 100 (not 0 and 1).")


###Testing_Incident_Tests
expect_true(inherits(combo$Testing_Incident_Tests, 'numeric'),
            info = "Testing_Incident_Tests column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Incident_Tests)), 0,
             info = "Testing_Incident_Tests column has no NA/missings")

###Testing_Incident_Test_Target
expect_true(inherits(combo$Testing_Incident_Test_Target, 'numeric'),
            info = "Testing_Incident_Test_Target column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Incident_Test_Target[combo$RowType == "Summary"])), 0,
             info = "Testing_Incident_Test_Target column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Testing_Incident_Test_Target[combo$RowType == "Daily"])), 0,
             info = "Testing_Incident_Test_Target column has *all* NA/missings for Daily Rows")

###Testing_Percent_of_Target
expect_true(inherits(combo$Testing_Percent_of_Target, 'numeric'),
            info = "Testing_Percent_of_Target column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Percent_of_Target[combo$RowType == "Summary"])), 0,
             info = "Testing_Percent_of_Target column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$Testing_Percent_of_Target[combo$RowType == "Daily"])), 0,
             info = "Testing_Percent_of_Target column has *all* NA/missings for Daily Rows")
expect_true(any(combo$Testing_Percent_of_Target[combo$RowType == "Summary"] > 1.0),
            info = "Testing_Percent_of_Target columns values are scales between 0 and 100+ (not 0 and 1+).")

###Testing_Composite_Class
expect_true(inherits(combo$Testing_Composite_Class, 'character'),
            info = "Testing_Composite_Class column is 'character' class")
expect_equal(sum(is.na(combo$Testing_Composite_Class[combo$RowType == "Summary"])), 0,
...
###Hosp_RunDate
expect_true(inherits(combo$Hosp_RunDate, 'Date'),
            info = "Hosp_RunDate column is 'Date' class")
expect_equal(sum(is.na(combo$Hosp_RunDate[combo$RowType == "Summary" & !(combo$Region %in% miss_hosp_cty)])), 0,
...
expect_equal(sum(is.na(combo$Hosp_RunDate[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
             info = "Hosp_RunDate column has no NA/missings for daily rows for state and HERC regions")
expect_equal(sum(!is.na(combo$Hosp_RunDate[combo$RowType == "Summary" & combo$Region %in% miss_hosp_cty])) +
             sum(!is.na(combo$Hosp_RunDate[combo$RowType == "Daily" & !(combo$Region %in% non_cty)])), 0,
             info = "Hosp_RunDate column is always NA/missings when expected")

###Hosp_dailyCOVID_px
expect_true(inherits(combo$Hosp_dailyCOVID_px, 'numeric'),
            info = "Hosp_dailyCOVID_px column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_dailyCOVID_px[combo$RowType == "Summary"])), 0,
             info = "Hosp_dailyCOVID_px column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_dailyCOVID_px[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(inherits(combo$Hosp_dailyCOVID_ICUpx, 'numeric'),
            info = "Hosp_dailyCOVID_ICUpx column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_dailyCOVID_ICUpx[combo$RowType == "Summary"])), 0,
             info = "Hosp_dailyCOVID_ICUpx column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_dailyCOVID_ICUpx[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
###Hosp_totalbeds
expect_true(inherits(combo$Hosp_totalbeds, 'numeric'),
            info = "Hosp_totalbeds column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_totalbeds[combo$RowType == "Summary"])), 0,
             info = "Hosp_totalbeds column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_totalbeds[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
###Hosp_beds_IBA
expect_true(inherits(combo$Hosp_beds_IBA, 'numeric'),
            info = "Hosp_beds_IBA column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_beds_IBA[combo$RowType == "Summary"])), 0,
             info = "Hosp_beds_IBA column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_beds_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
###Hosp_totalICU
expect_true(inherits(combo$Hosp_totalICU, 'numeric'),
            info = "Hosp_totalICU column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_totalICU[combo$RowType == "Summary"])), 0,
             info = "Hosp_totalICU column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_totalICU[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
###Hosp_ICU_IBA
expect_true(inherits(combo$Hosp_ICU_IBA, 'numeric'),
            info = "Hosp_ICU_IBA column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_ICU_IBA[combo$RowType == "Summary"])), 0,
             info = "Hosp_ICU_IBA column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_ICU_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
###Hosp_num_px_vent
expect_true(inherits(combo$Hosp_num_px_vent, 'numeric'),
            info = "Hosp_num_px_vent column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_num_px_vent[combo$RowType == "Summary"])), 0,
             info = "Hosp_num_px_vent column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_num_px_vent[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
###Hosp_total_vents
expect_true(inherits(combo$Hosp_total_vents, 'numeric'),
            info = "Hosp_total_vents column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_total_vents[combo$RowType == "Summary"])), 0,
             info = "Hosp_total_vents column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_total_vents[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(inherits(combo$Hosp_intermed_beds_IBA, 'numeric'),
            info = "Hosp_intermed_beds_IBA column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_intermed_beds_IBA[combo$RowType == "Summary"])), 0,
             info = "Hosp_intermed_beds_IBA column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_intermed_beds_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(inherits(combo$Hosp_negflow_beds_IBA, 'numeric'),
            info = "Hosp_negflow_beds_IBA column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_negflow_beds_IBA[combo$RowType == "Summary"])), 0,
             info = "Hosp_negflow_beds_IBA column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_negflow_beds_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(inherits(combo$Hosp_medsurg_beds_IBA, 'numeric'),
            info = "Hosp_medsurg_beds_IBA column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_medsurg_beds_IBA[combo$RowType == "Summary"])), 0,
             info = "Hosp_medsurg_beds_IBA column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_medsurg_beds_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
###Hosp_PrctBeds_IBA
expect_true(inherits(combo$Hosp_PrctBeds_IBA, 'numeric'),
            info = "Hosp_PrctBeds_IBA column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_PrctBeds_IBA[combo$RowType == "Summary"])), 0,
             info = "Hosp_PrctBeds_IBA column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_PrctBeds_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(all(dplyr::between(combo$Hosp_PrctBeds_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty], left = 0.0, right = 100.0)),
            info = "Hosp_PrctBeds_IBA columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$Hosp_PrctBeds_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty] > 1.0),
            info = "Hosp_PrctBeds_IBA columns values are scales between 0 and 100 (not 0 and 1).")

###Hosp_PrctICU_IBA
expect_true(inherits(combo$Hosp_PrctICU_IBA, 'numeric'),
            info = "Hosp_PrctICU_IBA column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_PrctICU_IBA[combo$RowType == "Summary"])), 0,
             info = "Hosp_PrctICU_IBA column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_PrctICU_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(all(dplyr::between(combo$Hosp_PrctICU_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty], left = 0.0, right = 100.0)),
            info = "Hosp_PrctICU_IBA columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$Hosp_PrctICU_IBA[combo$RowType == "Daily" & combo$Region %in% non_cty] > 1.0),
            info = "Hosp_PrctICU_IBA columns values are scales between 0 and 100 (not 0 and 1).")

###Hosp_PrctVent_Used
expect_true(inherits(combo$Hosp_PrctVent_Used, 'numeric'),
            info = "Hosp_PrctVent_Used column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_PrctVent_Used[combo$RowType == "Summary"])), 0,
             info = "Hosp_PrctVent_Used column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_PrctVent_Used[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(all(dplyr::between(combo$Hosp_PrctVent_Used[combo$RowType == "Daily" & combo$Region %in% non_cty], left = 0.0, right = 100.0)),
            info = "Hosp_PrctVent_Used columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$Hosp_PrctVent_Used[combo$RowType == "Daily" & combo$Region %in% non_cty] > 1.0),
            info = "Hosp_PrctVent_Used columns values are scales between 0 and 100 (not 0 and 1).")

###Hosp_COVID_px_Trajectory
expect_true(inherits(combo$Hosp_COVID_px_Trajectory, 'character'),
            info = "Hosp_COVID_px_Trajectory column is 'character' class")
expect_equal(sum(is.na(combo$Hosp_COVID_px_Trajectory[combo$RowType == "Summary" & !(combo$Region %in% miss_hosp_cty)])), 0,
...
expect_equal(sum(!is.na(combo$Hosp_COVID_px_Trajectory[combo$RowType == "Daily"])), 0,
...
expect_true(inherits(combo$Hosp_COVID_px_Trajectory_Class, 'character'),
            info = "Hosp_COVID_px_Trajectory_Class column is 'character' class")
expect_equal(sum(is.na(combo$Hosp_COVID_px_Trajectory_Class[combo$RowType == "Summary" & !(combo$Region %in% miss_hosp_cty)])), 0,
...
expect_equal(sum(!is.na(combo$Hosp_COVID_px_Trajectory_Class[combo$RowType == "Daily"])), 0,
...
expect_equal(sort(unique(combo$Hosp_COVID_px_Trajectory_Class[combo$RowType == "Summary" & !(combo$Region %in% miss_hosp_cty)])),
             c("Growing", "No significant change", "Shrinking"),
             info = "Hosp_COVID_px_Trajectory_Class column has only correct unique values")

###Hosp_COVID_ICUpx_Trajectory
expect_true(inherits(combo$Hosp_COVID_ICUpx_Trajectory, 'character'),
            info = "Hosp_COVID_ICUpx_Trajectory column is 'character' class")
expect_equal(sum(is.na(combo$Hosp_COVID_ICUpx_Trajectory[combo$RowType == "Summary" & !(combo$Region %in% miss_hosp_cty)])), 0,
...
expect_equal(sum(!is.na(combo$Hosp_COVID_ICUpx_Trajectory[combo$RowType == "Daily"])), 0,
...
expect_true(inherits(combo$Hosp_COVID_ICUpx_Trajectory_Class, 'character'),
            info = "Hosp_COVID_ICUpx_Trajectory_Class column is 'character' class")
expect_equal(sum(is.na(combo$Hosp_COVID_ICUpx_Trajectory_Class[combo$RowType == "Summary" & !(combo$Region %in% miss_hosp_cty)])), 0,
...
expect_equal(sum(!is.na(combo$Hosp_COVID_ICUpx_Trajectory_Class[combo$RowType == "Daily"])), 0,
...
expect_equal(sort(unique(combo$Hosp_COVID_ICUpx_Trajectory_Class[combo$RowType == "Summary" & !(combo$Region %in% miss_hosp_cty)])),
             c("Growing", "No significant change", "Shrinking"),
             info = "Hosp_COVID_ICUpx_Trajectory_Class column has only correct unique values")

###Hosp_PrctBeds_Used
expect_true(inherits(combo$Hosp_PrctBeds_Used, 'numeric'),
            info = "Hosp_PrctBeds_Used column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_PrctBeds_Used[combo$RowType == "Summary"])), 0,
             info = "Hosp_PrctBeds_Used column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_PrctBeds_Used[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(all(dplyr::between(combo$Hosp_PrctBeds_Used[combo$RowType == "Daily" & combo$Region %in% non_cty], left = 0.0, right = 100.0)),
            info = "Hosp_PrctBeds_Used columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$Hosp_PrctBeds_Used[combo$RowType == "Daily" & combo$Region %in% non_cty] > 1.0),
            info = "Hosp_PrctBeds_Used columns values are scales between 0 and 100 (not 0 and 1).")

###Hosp_PrctICU_Used
expect_true(inherits(combo$Hosp_PrctICU_Used, 'numeric'),
            info = "Hosp_PrctICU_Used column is 'numeric' class")
expect_equal(sum(!is.na(combo$Hosp_PrctICU_Used[combo$RowType == "Summary"])), 0,
             info = "Hosp_PrctICU_Used column has only NA/missings for summary")
expect_equal(sum(is.na(combo$Hosp_PrctICU_Used[combo$RowType == "Daily" & combo$Region %in% non_cty])), 0,
...
expect_true(all(dplyr::between(combo$Hosp_PrctICU_Used[combo$RowType == "Daily" & combo$Region %in% non_cty], left = 0.0, right = 100.0)),
            info = "Hosp_PrctICU_Used columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$Hosp_PrctICU_Used[combo$RowType == "Daily" & combo$Region %in% non_cty] > 1.0),
...
##. . CLI ----
###CLI_Count
expect_true(inherits(combo$CLI_Count, 'numeric'),
            info = "CLI_Count column is 'numeric' class")
expect_equal(sum(is.na(combo$CLI_Count)), 0,
             info = "CLI_Count column has no NA/missings")

###CLI_Burden
expect_true(inherits(combo$CLI_Burden, 'numeric'),
            info = "CLI_Burden column is 'numeric' class")
expect_equal(sum(is.na(combo$CLI_Burden[combo$RowType == "Summary"])), 0,
             info = "CLI_Burden column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$CLI_Burden[combo$RowType == "Daily"])), 0,
             info = "CLI_Burden column has *all* NA/missings for Daily Rows")

###CLI_Trajectory
expect_true(inherits(combo$CLI_Trajectory, 'character'),
            info = "CLI_Trajectory column is 'character' class")
expect_equal(sum(is.na(combo$CLI_Trajectory[combo$RowType == "Summary"])), 0,
             info = "CLI_Trajectory column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$CLI_Trajectory[combo$RowType == "Daily"])), 0,
             info = "CLI_Trajectory column has *all* NA/missings for Daily Rows")

###CLI_Burden_Class
expect_true(inherits(combo$CLI_Burden_Class, 'character'),
            info = "CLI_Burden_Class column is 'character' class")
expect_equal(sum(is.na(combo$CLI_Burden_Class[combo$RowType == "Summary"])), 0,
             info = "CLI_Burden_Class column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$CLI_Burden_Class[combo$RowType == "Daily"])), 0,
             info = "CLI_Burden_Class column has *all* NA/missings for Daily Rows")
expect_equal(sort(unique(combo$CLI_Burden_Class)),
             c("High", "Low", "Moderate", "Moderately High"),
             info = "CLI_Burden_Class column has only correct unique values")

###CLI_Trajectory_Class
expect_true(inherits(combo$CLI_Trajectory_Class, 'character'),
            info = "CLI_Trajectory_Class column is 'character' class")
expect_equal(sum(is.na(combo$CLI_Trajectory_Class[combo$RowType == "Summary"])), 0,
             info = "CLI_Trajectory_Class column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$CLI_Trajectory_Class[combo$RowType == "Daily"])), 0,
             info = "CLI_Trajectory_Class column has *all* NA/missings for Daily Rows")
expect_equal(sort(unique(combo$CLI_Trajectory_Class)),
             c("Growing", "No significant change", "Shrinking"),
             info = "CLI_Trajectory_Class column has only correct unique values")

###CLI_Composite_Class
expect_true(inherits(combo$CLI_Composite_Class, 'character'),
            info = "CLI_Composite_Class column is 'character' class")
expect_equal(sum(is.na(combo$CLI_Composite_Class[combo$RowType == "Summary"])), 0,
             info = "CLI_Composite_Class column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$CLI_Composite_Class[combo$RowType == "Daily"])), 0,
             info = "CLI_Composite_Class column has *all* NA/missings for Daily Rows")
expect_equal(sort(unique(combo$CLI_Composite_Class)),
             c("High", "Low", "Medium"),
             info = "CLI_Composite_Class column has only correct unique values")

###CLI_Trajectory_P
expect_true(inherits(combo$CLI_Trajectory_P, 'numeric'),
            info = "CLI_Trajectory_P column is 'numeric' class")
expect_equal(sum(is.na(combo$CLI_Trajectory_P[combo$RowType == "Summary"])), 0,
             info = "CLI_Trajectory_P column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$CLI_Trajectory_P[combo$RowType == "Daily"])), 0,
             info = "CLI_Trajectory_P column has *all* NA/missings for Daily Rows")
expect_true(all(dplyr::between(combo$CLI_Trajectory_P[combo$RowType == "Summary"], left = 0.0, right = 1.0)),
            info = "CLI_Trajectory_P columns values are all between 0 and 1 inclusive.")

###CLI_Trajectory_FDR
expect_true(inherits(combo$CLI_Trajectory_FDR, 'numeric'),
            info = "CLI_Trajectory_FDR column is 'numeric' class")
expect_equal(sum(is.na(combo$CLI_Trajectory_FDR[combo$RowType == "Summary"])), 0,
             info = "CLI_Trajectory_FDR column has no NA/missings for Summary Rows")
expect_equal(sum(!is.na(combo$CLI_Trajectory_FDR[combo$RowType == "Daily"])), 0,
             info = "CLI_Trajectory_FDR column has *all* NA/missings for Daily Rows")
expect_true(all(dplyr::between(combo$CLI_Trajectory_FDR[combo$RowType == "Summary"], left = 0.0, right = 1.0)),
            info = "CLI_Trajectory_FDR columns values are all between 0 and 1 inclusive.")

##. . ILI ----
###ILI_Total_Visits
expect_true(inherits(combo$ILI_Total_Visits, 'numeric'),
            info = "ILI_Total_Visits column is 'numeric' class")
expect_equal(sum(is.na(combo$ILI_Total_Visits[combo$RowType == "Daily"])), 0,
             info = "ILI_Total_Visits column has no NA/missings for Daily rows")
expect_equal(sum(!is.na(combo$ILI_Total_Visits[combo$RowType == "Summary"])), 0,
             info = "ILI_Total_Visits column has ONLY NA/missings for Summary rows")

###ILI_Visits
expect_true(inherits(combo$ILI_Visits, 'numeric'),
            info = "ILI_Visits column is 'numeric' class")
expect_equal(sum(is.na(combo$ILI_Visits[combo$RowType == "Daily"])), 0,
             info = "ILI_Visits column has no NA/missings for Daily rows")
expect_equal(sum(!is.na(combo$ILI_Visits[combo$RowType == "Summary"])), 0,
             info = "ILI_Visits column has ONLY NA/missings for Summary rows")

###ILI_Percent
expect_true(inherits(combo$ILI_Percent, 'numeric'),
            info = "ILI_Percent column is 'numeric' class")
expect_equal(sum(is.na(combo$ILI_Percent[combo$RowType == "Daily"])), 0,
             info = "ILI_Percent column has no NA/missings for Daily rows")
expect_equal(sum(!is.na(combo$ILI_Percent[combo$RowType == "Summary"])), 0,
             info = "ILI_Percent column has ONLY NA/missings for Summary rows")
expect_true(all(dplyr::between(combo$ILI_Percent[combo$RowType == "Daily"], left = 0.0, right = 100.0)),
            info = "ILI_Percent columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$ILI_Percent[combo$RowType == "Daily"] > 1.0),
            info = "ILI_Percent columns values are scales between 0 and 100 (not 0 and 1).")

###ILI_Baseline
expect_true(inherits(combo$ILI_Baseline, 'numeric'),
            info = "ILI_Baseline column is 'numeric' class")
expect_equal(sum(is.na(combo$ILI_Baseline[combo$RowType == "Daily" & combo$Region != "Florence"])), 0,
             info = "ILI_Baseline column has no NA/missings for Daily rows outside of Florence County")
expect_equal(sum(!is.na(combo$ILI_Baseline[combo$RowType == "Summary" | combo$Region == "Florence"])), 0,
             info = "ILI_Baseline column has ONLY NA/missings for Summary rows and Florence County")
expect_true(all(dplyr::between(combo$ILI_Baseline[combo$RowType == "Daily"], left = 0.0, right = 100.0)),
            info = "ILI_Baseline columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$ILI_Baseline[combo$RowType == "Daily"] > 1.0),
            info = "ILI_Baseline columns values are scales between 0 and 100 (not 0 and 1).")

###ILI_Threshold
expect_true(inherits(combo$ILI_Threshold, 'numeric'),
            info = "ILI_Threshold column is 'numeric' class")
expect_equal(sum(is.na(combo$ILI_Threshold[combo$RowType == "Daily" & combo$Region != "Florence"])), 0,
             info = "ILI_Threshold column has no NA/missings for Daily rows outside of Florence County")
expect_equal(sum(!is.na(combo$ILI_Threshold[combo$RowType == "Summary" | combo$Region == "Florence"])), 0,
             info = "ILI_Threshold column has ONLY NA/missings for Summary rows and Florence County")
expect_true(all(dplyr::between(combo$ILI_Threshold[combo$RowType == "Daily"], left = 0.0, right = 100.0)),
            info = "ILI_Threshold columns values are all between 0 and 100 inclusive.")
expect_true(any(combo$ILI_Threshold[combo$RowType == "Daily"] > 1.0),
...
###Testing_Case
expect_true(inherits(combo$Testing_Case, 'numeric'),
            info = "Testing_Case column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Case[combo$RowType == "Summary"])), 0,
             info = "Testing_Case column has no NA/missings for Summary rows")
expect_equal(sum(!is.na(combo$Testing_Case[combo$RowType == "Daily"])), 0,
             info = "Testing_Case column has ONLY NA/missings for Daily rows")

###Testing_ARI
expect_true(inherits(combo$Testing_ARI, 'numeric'),
            info = "Testing_ARI column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_ARI[combo$RowType == "Summary"])), 0,
             info = "Testing_ARI column has no NA/missings for Summary rows")
expect_equal(sum(!is.na(combo$Testing_ARI[combo$RowType == "Daily"])), 0,
             info = "Testing_ARI column has ONLY NA/missings for Daily rows")

###Testing_Case_Gap
expect_true(inherits(combo$Testing_Case_Gap, 'numeric'),
            info = "Testing_Case_Gap column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Case_Gap[combo$RowType == "Summary"])), 0,
             info = "Testing_Case_Gap column has no NA/missings for Summary rows")
expect_equal(sum(!is.na(combo$Testing_Case_Gap[combo$RowType == "Daily"])), 0,
             info = "Testing_Case_Gap column has ONLY NA/missings for Daily rows")

###Testing_Target_0.2
expect_true(inherits(combo$Testing_Target_0.2, 'numeric'),
            info = "Testing_Target_0.2 column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Target_0.2[combo$RowType == "Summary"])), 0,
             info = "Testing_Target_0.2 column has no NA/missings for Summary rows")
expect_equal(sum(!is.na(combo$Testing_Target_0.2[combo$RowType == "Daily"])), 0,
             info = "Testing_Target_0.2 column has ONLY NA/missings for Daily rows")

###Testing_Target_0.4
expect_true(inherits(combo$Testing_Target_0.4, 'numeric'),
            info = "Testing_Target_0.4 column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Target_0.4[combo$RowType == "Summary"])), 0,
             info = "Testing_Target_0.4 column has no NA/missings for Summary rows")
expect_equal(sum(!is.na(combo$Testing_Target_0.4[combo$RowType == "Daily"])), 0,
             info = "Testing_Target_0.4 column has ONLY NA/missings for Daily rows")

###Testing_Target_0.6
expect_true(inherits(combo$Testing_Target_0.6, 'numeric'),
            info = "Testing_Target_0.6 column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Target_0.6[combo$RowType == "Summary"])), 0,
             info = "Testing_Target_0.6 column has no NA/missings for Summary rows")
expect_equal(sum(!is.na(combo$Testing_Target_0.6[combo$RowType == "Daily"])), 0,
             info = "Testing_Target_0.6 column has ONLY NA/missings for Daily rows")

###Testing_Target_0.8
expect_true(inherits(combo$Testing_Target_0.8, 'numeric'),
            info = "Testing_Target_0.8 column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Target_0.8[combo$RowType == "Summary"])), 0,
             info = "Testing_Target_0.8 column has no NA/missings for Summary rows")
expect_equal(sum(!is.na(combo$Testing_Target_0.8[combo$RowType == "Daily"])), 0,
             info = "Testing_Target_0.8 column has ONLY NA/missings for Daily rows")

###Testing_Target_1
expect_true(inherits(combo$Testing_Target_1, 'numeric'),
            info = "Testing_Target_1 column is 'numeric' class")
expect_equal(sum(is.na(combo$Testing_Target_1[combo$RowType == "Summary"])), 0,
             info = "Testing_Target_1 column has no NA/missings for Summary rows")
expect_equal(sum(!is.na(combo$Testing_Target_1[combo$RowType == "Daily"])), 0,
