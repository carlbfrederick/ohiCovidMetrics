...
```{r pull-hist-table}
hdt_raw <- pull_histTable() %>% 
  filter(post_date <= as.Date("2020-06-17"))
```

This function does four things: 

1. Imports data from the REST API <https://services1.arcgis.com/ISZ89Z51ft1G16OK/ArcGIS/rest/services/COVID19_WI/FeatureServer/10/query?where=GEO%20%3D%20'STATE'%20OR%20GEO%20%3D%20'COUNTY'&outFields=GEOID,GEO,NAME,LoadDttm,NEGATIVE,POSITIVE,DEATHS,TEST_NEW,POS_NEW,DTH_NEW&outSR=4326&f=geojson> 
...
4. Finally, it produces a cumulative case time-series based on the cleaned version. 

[^1]: It pulls down the new deaths and tests time series as well for more context.

The result is a data.frame where there is one row per county/state per day.
...
4. Appends the region populations using the `county_data` data.frame
   that is built into the package.
5. Aggregates the counts for each HERC region and appends it to the
   bottom of the output data.frame so that there are 80 rows (72 
   counties + 1 state + 7 HERC regions).
...
wrapped all of these calcuations into the `process_confirmed_cases()`
