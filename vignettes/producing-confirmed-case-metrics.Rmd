...
hdt_cusum_cty <- hdt_raw %>% 
  ...
  dplyr::left_join(dplyr::select(county_data, fips, herc_region), by = "fips")

herc <- hdt_cusum_cty %>% 
   dplyr::filter(fips != "55") %>%
   dplyr::group_by(week_num, herc_region) %>%
   dplyr::summarize(
     case_weekly = sum(case_weekly),
...
hdt_cusum <- bind_rows(hdt_cusum_cty, herc) %>% 
  group_by(fips, geo_name) %>% 
